...
- A `res://` value is checked against the filesystem, the way `project_set_autoload` checks a script. `application/run/main_scene` could be pointed at a file that does not exist and the write reported success, leaving a project that no longer runs. A path with nothing at it is a `404` carrying `resource_exists: false`. The check applies only when the whole value is a string; paths inside an array are not checked, live or offline (#989). `internationalization/locale/translations` is an array, so it accepts a missing file, a JSON file, or the `.csv` the translations were imported from, and each is reported as written. With the `.csv` registered, a game loads no translation and prints one ERROR at startup. Register the `.translation` files the CSV's import sidecar lists under `dest_files`.
