...
**The editor writes the file, and the result says whether it has.** The bus goes into the editor's running `AudioServer`, and the editor's own bus-layout autosave writes the layout 0.8 to 0.9 s later, to the path `audio/buses/default_bus_layout` names. The tool waits up to five seconds for that write, which is headroom for an editor drawing slowly: on a CI runner rendering in software, the first write of a new layout file on 4.5.1 took longer than two. It reads the file back through the same reader `audio_list_buses` uses offline: `layout_written: true` means the bus is in it at the index it was given. `layout_written: false` comes with a `layout_note` naming what keeps a write from arriving: an editor still running slowly, a read-only layout file (the editor prints "Safe save failed", which `runtime_read_output` shows), or an editor that is writing the layout it opened the project with, which it goes on doing after `audio/buses/default_bus_layout` changes until it restarts. `layout_path` is read from the project setting, not assumed. On 4.5.1 and 4.6.2 the editor's Audio panel does not follow a rename, so a new strip would say `New Bus`, and one click into that name field and away would rename the bus back; the tool has the panel rebuild once the bus is named, and `panel_refreshed` says it did.
