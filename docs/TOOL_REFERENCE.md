...
- `script_path` (`string`, required); must end in `.gd`, hold no control characters, and resolve inside the project root. Containment is decided by resolving the path, so `res://nested/../ok.gd` is the project-root file it names rather than a refusal. A scheme other than `res://` is refused, by this tool and by every other one that writes a file: `user://` is the running game's data directory, outside the project, and `RES://` is not Godot's spelling, since its scheme is lower case. Both used to be read as a folder inside the project named `user:` or `RES:`.
...
**The editor's copy.** The editor keeps every resource it has loaded, and nothing an unattended editor does re-reads a file that changed underneath it: not `editor_reload_project`, and not the editor's own filesystem scan. So the library is compared with the file first, by each animation's name, length, loop mode and tracks (type, path and key count). When the file was rewritten after the editor loaded it by a writer the editor never heard about, such as an agent's own file tools, the call is `409 animation_library_differs_from_disk`, naming the animations each copy holds, with `retry_with: {reload_from_disk: true}`. With `reload_from_disk` the editor's copy is reloaded in place from the file before the add, so every player already holding the library shows the file's version too, and the result says `reloaded_from_disk: true`. Reloading discards any change made to the editor's copy and not saved, which is why it is asked for rather than done. Key times and values are not compared. A rewrite through `resource_create` with the editor attached does not get this far: that tool reloads the editor's copy of every file it writes.
...
- `target` (`string`, required, 1-256 characters). A canonical `res://` path, lowercase-alphanumeric `uid://` value, static Godot node path such as `.`, `..`, `Player/Sprite`, `/root`, `%Player`, `Hand/Sword/%Hilt`, or `$Player/Sprite`, or a single Godot identifier. Quoted calls may contain valid spaces, punctuation, or UTF-8 node names.
...
Accepts `paths` containing 1–256 unique normalized `res://` source files and `timeout_ms` from 1–10,000. The editor revalidates the whole batch, rejects `.godot`, `.import`, directories, and missing/out-of-project files, and allows one pending reimport. Success requires two consecutive main-loop callbacks with `is_scanning() == false`. A timeout returns `504` with an unknown outcome because Godot may finish afterward, and says what it was still waiting for: `editor_scanning` under `error.data`, and `scan_applied` when the call asked for a scan. A slow editor spends most of a scan applying it, because each new script is registered under a progress task that draws a frame per step; one drawing in software took longer than ten seconds over 25 scripts.
...
A path with no sidecar is two different things, and this tool is the one that tells them apart. A `.gd`, a `.tscn` or a `.tres` never gets a sidecar and `update_file` is all it needs. An asset the editor has never scanned -- a `.png` written into the project by something other than Godot -- needs importing before anything can use it, and `update_file` does not import: it announces. So whenever a path lacks a sidecar and the editor has not finished with it -- it does not list the file, or the file's import is outstanding -- the call also runs `EditorFileSystem.scan`, which is the walk that finds new files and runs the importer over them. A script the editor already lists needs `update_file` and no more. `editor_reload_project` uses `scan_sources`, which only re-examines files the editor already knows about, and a file it has never seen is not one of those.
...
Nor does it answer from inside an import pass. A windowed editor runs its main loop from inside one, because its progress dialog does, so Didi runs in the middle of the pass. The sidecars and the scanning flag settle before the pass has emitted `resources_reimported`, which is what reloads the resources a caller reads next, and a caller answered then sent its next reimport into the pass, where Godot opened a second progress task over its own and printed three errors (#914). So while a pass is open, whoever started it, the call is not answered and Didi takes no request off its queue. The pass is open from `resources_reimporting` to `resources_reimported`, which the addon's `didi_import_watch.gd` counts, and on Godot 4.7 also while `EditorFileSystem.is_importing()` is true. That flag alone is not enough: the editor clears it before its last progress task closes. `timeout_ms` still ends the wait, and a timeout inside a pass carries `editor_import_pass_open: true` under `error.data`. Godot refuses to start a reimport while its importing flag is set and reports that only in its log, and the call answers `409 editor_import_busy`, retryable, with the engine's line under `engine_diagnostics`. For a project whose copy of the addon has no `didi_import_watch.gd`, the extension prints one warning to the editor's console and falls back to what the engine supplies alone: Didi's own passes, and the 4.7 flag.

Nor does it reimport while the editor scans. `reimport_files` looks each file up in the editor's file index, and while a scan runs it finds none: the engine prints `Can't find file ... during file reimport`, skips the file, and returns nothing. A batch that paired an asset with a file needing a scan -- a new script, an asset the editor had never seen -- started the reimport straight after the scan, and in a project whose scan outlasts the call it reimported nothing while answering that it had; with 800 scripts in the project that was every such batch, and the editor sometimes crashed. An asset named while the editor's own scan ran met the same. The scanning flag is not the end of it: it clears on the scan's own thread, and a later frame swaps in the new index, updates script classes and their documentation under progress tasks that run frames of their own, and then emits `sources_changed`. A reimport started in between had the old index freed under it and collided with those tasks, with hundreds of `Task ... already exists` lines. So the reimport is held while a scan runs, until the editor has emitted `sources_changed` for it (which `didi_import_watch.gd` counts, with the index each emission was made with), while a progress task is open, and until the editor lists every asset; the frame loop then starts it. A call that asked for a scan is likewise not answered until `sources_changed`, so the caller's next request does not land in the frames that apply the scan. Nothing is reimported when the wait ends any other way: an asset the editor still does not list once its scan is over is `409 asset_not_indexed` with `not_indexed`, and a scan that outlasts `timeout_ms` is `504 editor_scanning`, retryable, both with `outcome: not_imported`. `tools/vibe/probes/scan_reimport_engine.py` asked the engine alone on 4.5.1, 4.6.2 and 4.7.2, and `tools/vibe/probes/mixed_reimport.py` asks the tool.
...
Sets a bus volume, mute or solo on the running engine: the attached editor's, or a running game's. In a game it changes what is being heard now and nothing else, which is the right tool for turning the music down while a game is played.
...
**This tool writes no file, and in an attached editor the change reaches disk anyway.** The editor's own bus-layout autosave notices the `AudioServer` change and writes the project's bus layout file a moment later, the path `audio/buses/default_bus_layout` names (`res://default_bus_layout.tres` unless the project moved it), with no further call from anybody, so a tracked project file appears in the working tree carrying whatever value was tried last. `persisted_by_editor` says whether that will happen, `layout_path` names the file, and `limitation` says it in words. The file is the one the editor opened the project with: its Audio panel reads `audio/buses/default_bus_layout` once, when it is built, and saves there until the editor restarts, so after the setting has moved underneath it the result also carries `project_layout_path`, the file the next start loads, and `limitation` says the two differ. A read-only layout file makes the editor's save fail on Windows ("Safe save failed"), so there the result carries `layout_read_only: true` and `persisted_by_editor: false`; elsewhere `layout_read_only` is reported and the claim kept, because a POSIX rename can replace a read-only file. A game session gets the opposite sentence: nothing writes the change down there and it is gone when the process exits. There is also a window, a few seconds wide, in which `audio_list_buses` answers differently depending on whether an editor is attached, and then the window closes and the two agree.
...
### `audio_add_bus` — Live (editor only)

Adds one bus to the end of the layout the attached editor holds, names it and routes it. A fresh project has only `Master`, and a settings menu needs a `Music` and an `SFX` bus to set; until a bus has a name, `audio_configure_bus` answers `404` for it and an `AudioStreamPlayer`'s `bus` reads back `Master`.

- `name` (required, 1 to 256 characters). The bus's name.
- `send` (default `Master`). The bus it feeds into.
- `volume_db` (`number`, -80 to 24), `mute` (`boolean`), `solo` (`boolean`). The new bus's starting values; 0 dB, unmuted and not soloed otherwise.

Godot's `AudioServer` never refuses a name or a send, measured on 4.5.1, 4.6.2 and 4.7.2, so every refusal here is one the engine would otherwise have answered by changing what it was given:

- A name already in use is refused `409` with `data.code: "bus_name_in_use"` and the list of buses. The engine would name the new bus `Music 2`, and the name asked for would belong to the other one. `Master` is in use on every project. The name is read back after it is set, and if the engine changed it anyway, the bus is removed again. If another bus holds the name by then, something else in the editor added it between the check and the call, and the same `bus_name_in_use` comes back; otherwise the engine changed the name on its way in, and the code is `bus_name_changed_by_engine` with the name it stored under `stored_as`. Neither is retryable.
- A name differing from one in use only in letter case is refused `409` with `bus_name_differs_only_in_case` and the bus it collides with. The engine keeps both, and a person cannot tell them apart in the Audio panel.
- An empty name, spaces alone, a leading or trailing space, and a control character are refused `400` naming `name`; a spaced name carries the trimmed one in `retry_with`. The engine keeps all of these. The rules are about what the Audio panel shows, not about ASCII: a space is anything Unicode calls white space, such as a no-break space or an ideographic space; a character drawn as nothing, such as a zero width space or a byte-order mark, counts as one; and a control character includes the C1 range, the line and paragraph separators and the bidirectional controls, which reorder how the rest of the name is drawn. `data.character` names the code point found, since it is the one thing the caller cannot see. Quotes, backslashes, slashes, brackets and non-ASCII text are ordinary names and are accepted, and so is a space or a joiner inside a name: an emoji ending in its variation selector and one held together by zero width joiners are names.
- A `send` naming no bus is refused `404` with `send_bus_not_found`, the list of buses, and `did_you_mean` when one differs only in case. A send to the new bus's own name is refused `400`, and so is the empty send, with `Master` in `retry_with`. The engine stores any of these, reads it back as set, and routes the sound to Master with no message. The new bus goes on the end, which is the only place a send works from: every bus already there comes before it, so any of them is a valid target. There is no position argument for the same reason.

The result has the bus's `index` under `bus`, the state read back from the engine under `after`, and `adopted_sends`: the buses that already sent to this name. A send is kept by name and does not follow a rename, so a project can hold a send to a name no bus has, and adding a bus with that name routes those buses through it.

**The editor writes the file, and the result says whether it has.** The bus goes into the editor's running `AudioServer`, and the editor's own bus-layout autosave writes the layout 0.8 to 0.9 s later, to the layout file the editor opened the project with, which is the one `audio/buses/default_bus_layout` names unless the setting moved while the editor was running; then `project_layout_path` names the other. The tool waits up to five seconds for that write, which is headroom for an editor drawing slowly: on a CI runner rendering in software, the first write of a new layout file on 4.5.1 took longer than two. It reads the file back through the same reader `audio_list_buses` uses offline: `layout_written: true` means the bus is in it at the index it was given. `layout_written: false` comes with a `layout_note`. A read-only layout file is named as the reason, with `layout_read_only: true`: the editor's autosave cannot replace it, prints "Safe save failed", which `runtime_read_output` shows, and the bus stays in the running editor only; on Windows the tool does not wait for a save that has already failed. Otherwise the note says a slow editor may still write it. `layout_path` is read from the editor's own startup setting, not assumed. On 4.5.1 and 4.6.2 the editor's Audio panel does not follow a rename, so a new strip would say `New Bus`, and one click into that name field and away would rename the bus back; the tool has the panel rebuild once the bus is named, and `panel_refreshed` says it did.

Add the bus before setting a player to it. A player whose `bus` names a bus that does not exist reads back `Master`, and a scene saved then has no `bus` line at all; after the bus exists, `scene_set_property` answers `applied: true` and the saved scene keeps the name.

Editor sessions only. With no editor attached the tool refuses and points at `audio_list_buses`; adding a bus offline would mean writing the layout file behind whatever editor later opens the project, which writes its own layout over it. A game session is refused, because a running game loaded its layout when it started and a bus added to it is in no file. Classified as a mutation that only adds: a dry run asks the editor and reports the buses and the index the bus would get, and no confirmation token is needed. There is no undo entry, `undo_redo_registered` is `false`, and the way back is deleting the bus in the editor's Audio panel.
...
Every event is constructed and fully configured on the Godot main thread before the first one is dispatched, so a malformed or unconstructible event anywhere in the batch fails the whole call with nothing sent. An `action` event must name an action the game's `InputMap` defines. Godot dispatches an `InputEventAction` for an action nobody declared without complaint and nothing reacts to it, so the call used to answer `completed` for a misspelled action exactly as for a real one. It is refused with `400`, `reason: "undefined_input_action"` and `undefined_actions`, before anything in the batch is sent, which is how `runtime_explore_scene` already refused the same batch. A game loads its actions when it starts, so an action written with `project_set_input_action` while the game was running is not in its map until it is restarted, and the refusal says so. Press and release are separate events; there is no duration, no timer and no implied release. `parse_input_event` returns void, so `dispatched_event_count` counts calls made, not events the game accepted. The response is `{dispatched_event_count, queued_event_count, event_types, outcome: "completed", rollback: "not_available", paused: false, delivery: "immediate", execution_mode: "live", session_kind: "game"}`.
...
- `audio/buses/default_bus_layout` moved while an editor is attached answers `requires_editor_restart: true`, `editor_layout_path` and a `limitation`. The editor's Audio panel read the setting when it was built and saves every bus change to that file until it restarts, while the next start loads the file the setting now names; moving it to a new, empty layout lost every bus the editor held at the next start, measured on 4.7.2 in vibe session nineteen. `editor_reload_project` is a filesystem scan and does not change it. Putting the setting back answers without the flag.
- A `res://` value is checked against the filesystem, the way `project_set_autoload` checks a script. `application/run/main_scene` could be pointed at a file that does not exist and the write reported success, leaving a project that no longer runs. A path with nothing at it is a `404` carrying `resource_exists: false`, with or without an editor. Every `res://` string inside an array is checked the same way, and the refusal adds `index`, the element's place in the array (#989). The array's checks need no engine, so they run before the route is chosen: with an editor attached, offline, and for a dry run alike. A path that names another scheme, such as `user://`, is not the project's and is not checked; one that resolves outside the project is a `400`.
- `internationalization/locale/translations` takes only the files Godot registers a translation from: a `.translation`, which is what a CSV import writes, one per locale beside the CSV; a `.po`; a `.mo`; or a `.res`. Each kind was registered alone on 4.5.1, 4.6.2 and 4.7.2 with the game told to use its locale, and those four loaded while nothing else did. The CSV itself loads nothing and prints an ERROR at startup. It is refused `409` with `data.code: "translation_source_registered"`, `translation_files` from its sidecar's `dest_files`, and `retry_with: {"value": [...]}`, which is the same list with the CSV replaced by those files. A CSV nothing has imported is `translation_source_not_imported`. A `.tres` is refused as `not_a_translation` even when it holds a Translation, because the engine loads registered translations before it can read a text resource and prints `No loader found`. So is every other kind of file, a `.json` among them, which the engine drops without a word. A `.res` or `.translation` is accepted by its extension; its contents are not opened to check that it holds a Translation, and a `uid://` entry is not checked.

`project_set_setting` is the one project writer with an offline route. With an editor attached it goes through `ProjectSettings` as before. With no session attached it edits `project.godot` directly and reports `execution_mode: "offline_fallback"`, `written_to`, `section`, `key`, `value_written`, `previous_value` when it replaced one, plus `section_created` and `replaced_existing`. The same name rules and the same value reach apply in both modes, so a value written offline is the Variant a live write would have stored. A section header is matched on the text inside the brackets, trimmed, so `[ application ]` is the application section and a hand-spaced file is updated in place rather than gaining a second copy of the section. The key is matched the way Godot builds it, so `config / name` is `application/config/name` and is replaced in place; a key an earlier line joined into is not this setting, so it reads as absent and the write lands as a new line at the end of the section, which is where the engine will read it. A value that spans lines is replaced whole, and `previous_value` is that whole value rather than the first line of it; a `; note` on a value's line is a comment for Godot and is not part of the value here either. A `project.godot` that ends part-way through a value is refused with a 409 by the write and by the dry run alike, because Godot answers `ERR_PARSE_ERROR` for that file and the project does not open: rewriting one line of it would report success and leave it exactly as unloadable. A file that is balanced and still unloadable is refused the same way and for the same reason, naming the line and the value the parser cannot start; the reach of that check is `unloadable_setting_value` above. A line that holds more than one key is refused too: `a=1 b=2` is two settings the engine reads, this writes whole lines, and changing one of them used to delete the other and report success about the one it was asked for. The refusal names the sibling, and splitting the line is the remedy. So is a key Godot built by joining the line above into this one, because that name lives on more lines than the rewrite replaces. A file that ends part-way through a *key* is the opposite case, which the engine drops with `OK`, so an ordinary trailing note is still written to. One check cannot cross over: there is no engine to ask whether a name is defined, and the shipped class reference publishes no `ProjectSettings` property list, so an offline write reports `defined_by_engine: null`, says so in `limitation`, and does not refuse. `create` is not needed offline for that reason. The offline write holds a lock on `project.godot` under `.didi/locks` from its read to its write, so two servers writing one project take turns rather than one replacing the other's setting; a write held off for five seconds is refused `409` with `data.code: "project_file_busy"` and `retryable: true`, and nothing is written. Bootstrapping the addon still works exactly as below.
...
Requires `session_id`. Didi connects to the exact validated process-unique endpoint and performs a token-authenticated protocol `1.3` handshake on a finite deadline of 3,000 ms plus the transport's idle-recycle window, which is 1,000 ms on Windows and 5,000 ms on POSIX. A server listens on four connections at once, so an arriving connection is normally read the moment it arrives. The window is still allowed for, because every slot can be held by a client that walked away without closing, and because the addon binary on the other end is updated separately from this one and may predate that. The token is inserted only into the internal envelope and stripped before bridge dispatch, responses, logs, and diagnostics. Route replacement is transactional: connection, authentication, ID, or protocol failure leaves the previous session selected.
...
The call holds a lock on `export_presets.cfg` under `.didi/locks` from its read of the file to its read-back, so two servers adding presets at once take turns and both presets land. A call held off for five seconds is refused `409` with `data.code: "project_file_busy"` and `retryable: true`, and the file is not touched. The dry run only reads, and takes no lock.
...
`project_export` with the same name and `mode: "pack"` then writes a `.pck` with no export templates installed, for every one of the seven platforms. A release or debug build needs the templates for the preset's platform, and `project_export` refuses with `export_templates_missing` when they are not there.
...
Godot trims the whitespace from both ends of every argument on its command line and then turns each `%20` in one back into a space, on 4.5.1, 4.6.2 and 4.7.2. So the preset name is sent with each space as `%20`: a name with a space at either end reached Godot as a different name and was not detected, and a name of spaces alone reached it empty, which started an editor that ran until the export timed out. A name that contains `%20` itself cannot be sent at all, and is refused at the dry run and on the call with `422` and `reason: "name_not_passable"` before Godot starts.

When Godot refuses the preset because of configuration errors, the block it prints under "due to configuration errors" is read into `configuration_errors`, one entry per problem. Missing export templates is the usual one, since a release or debug build needs them and a pack does not: it is `503` with `code: "toolchain_unavailable"` and `reason: "export_templates_missing"`, `missing_templates` holding the paths Godot looked in, the message naming the one this build needed, and `retry_with: {mode: "pack"}`. It was `500 internal_error`, with the cause left in `engine_output`. Any other configuration error, such as Android's SDK checks with its templates installed, is `422` with `reason: "export_configuration_errors"`.

A preset the file declares and Godot will never detect is refused the same way, at the dry run and on the call, with `422` before any Godot starts: `error.data` carries the `reason` from the table above, its `missing_index`, `section` or `did_you_mean`, the preset's `platform`, and `detected_presets`, the presets Godot will read. `available_presets` on a `404` lists only the presets the call would take. A platform Godot does not ship is the exception, because only the engine knows whether a plugin registered it: the call hands it to Godot, and the dry run says which case it is by carrying `not_detected` in the change's `before`. When Godot then does not detect it, the refusal is `404` with `reason: "not_detected_by_engine"`, `detected_presets` read from Godot's own list, and the console output as `engine_output`, rather than `500 internal_error`. Godot names the preset it looked for, and when that is not the one it was handed, the name did not survive its command line: the refusal says so and carries the name as `engine_looked_for`, rather than saying the file changed while the export ran. That is how a preset the editor was given with a name like `--verbose` fails, because Godot takes it as its own option and then reads the output path as the preset.
