...
The preview opens its target where it can. A dry-run against a script, resource, node or setting that is not there returns the same failure the real call would, so a preview cannot approve a call that can never execute. `target_read` and `preview_kind` say which happened: `target_state` with `changes[].kind: "planned_mutation"` and a `before` holding current state, or `argument_binding` with `changes[].kind: "unverified_mutation"` when the tool has no probe or the engine could not be reached. Reading a live node sends one read-only property read and changes nothing.
