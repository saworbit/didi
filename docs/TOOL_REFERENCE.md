...
- `name` (required, 1 to 256 bytes). The bus's name.
...
- A name already in use is refused `409` with `data.code: "bus_name_in_use"` and the list of buses. The engine would name the new bus `Music 2`, and the name asked for would belong to the other one. `Master` is in use on every project. The name is read back after it is set, and if the engine changed it anyway, the bus is removed again and the same code comes back with `retryable: true`.
...
- An empty name, spaces alone, a leading or trailing space, and a control character are refused `400` naming `name`; a spaced name carries the trimmed one in `retry_with`. The engine keeps all of these. Quotes, backslashes, slashes, brackets and non-ASCII text are ordinary names and are accepted.
