...
`project_search_text` reads the text formats a project keeps references in: `.gd`, `.cs`, `.tscn`, `.tres`, `.gdshader`, `.gdshaderinc`, `.godot`, `.cfg`, `.json` and `.import`. `project_search_symbols` reads only `.gd`, `.cs`, `.tscn` and `.tres`, because a declaration does not live anywhere else. Both report `unsearchable_files` and `unsearchable_extensions` for what was never a candidate, which is what tells an empty result apart from a string the project does not contain; `skipped_files` stays what it was, a file that was a candidate and could not be read. Both work beneath a normalized in-project `search_path`. They reject traversal/absolute paths, skip symlinks plus `.git`, `.godot`, `.gemini`, `.worktrees`, `.vs`, `out`, `bin`, `build` and any directory whose name starts with `build-`, and cap each file at 4 MiB, each request at 10,000 files/64 MiB, results at 500, queries at 256 UTF-8 bytes, and previews at 1,024 bytes.
