...
These are new to the bridge. `EditorFileSystem.reimport_files`,
`ResourceLoader.load` and `Object.get`, which reads the WAV's `loop_mode`,
`loop_begin` and `loop_end` back, are already bound.
...
- The empty name, spaces only, a leading or trailing space, and a control
  ...
  names.
- At most 256 characters, the bound preset names have and the unit the
  schema's `maxLength` counts in. The engine kept 300 letters, so the bound is
  Didi's. As first built these two bounds counted bytes; vibe session nineteen
  found that, and a no-break space, a zero width space and U+0085 accepted by
  rules written for ASCII. A space is now anything Unicode calls white space,
  a character drawn as nothing counts as one, and a control character includes
  the C1 range, the line and paragraph separators and the bidirectional
  controls (`AudioAddBus.NamesAsAPersonSeesThem`).
...
| **Proving test** | Native: `AnimAddLibrary.RequestValidation` covers the argument bounds, the `res://` path shape, the `.tres`/`.res` suffix and the four characters the engine refuses in a library name; `AnimAddLibrary.Registration` requires it to be live only, editor only, a mutation with `dry_run` and no confirmation token, additive and not read-only; `AnimAddLibrary.NoEditorNoCall` requires the call to be refused without an engine rather than answered, its dry run to say it read nothing rather than describe a plan, and a game session to be refused at the hook. Godot integration: requests 2600 to 2640 run in two editor batches against a scene of their own. A dry run reports the player's libraries and changes nothing; the real call adds a checked-in fixture library under a name; `anim_list_tracks` then lists its animation under the name `anim_play_track` takes; the saved scene file references the library as an `ExtResource`; `editor_undo` removes it and `editor_redo` puts it back, each read back through `anim_list_tracks`; and a name already in use, the same library under a second name, a name the engine refuses, a resource that is not an `AnimationLibrary`, a missing file, a malformed path and a node that is not an `AnimationPlayer` are each refused, with the name-in-use refusal also asserted on the dry run. Added after vibe session seventeen: a path in the wrong letter case, `scene_set_property` on the `libraries` slot for real and as a dry run, `scene_call_method add_animation_library`, and a second batch that rewrites the library file underneath the editor and requires the add to refuse, the dry run to reload nothing, `reload_from_disk` to add the file's version, and the player already holding the library to show it. |
...
**The editor's copy, and the file's spelling.** Two findings from vibe session
seventeen, fixed before the tool shipped. The editor keeps every resource it
has loaded and nothing an unattended editor does re-reads one: not
`editor_reload_project`, and not `EditorFileSystem.update_file` or `scan`,
measured by rewriting a library and reading a player after each. So the tool
compares the editor's copy with an uncached deep load of the file
(`CACHE_MODE_IGNORE_DEEP`) and refuses when they differ, and
`reload_from_disk: true` reloads the copy in place (`CACHE_MODE_REPLACE_DEEP`,
which keeps the objects' identity, so a player already holding the library
shows the new version). Both modes behave identically on 4.5.1, 4.6.2 and
4.7.2. The reload is asked for rather than automatic because it discards any
unsaved change to the editor's copy. And a path whose letter case differs
from the file on disk is refused with the on-disk spelling: Windows opens it,
the loader caches a second copy under that spelling, and the saved scene
...
| **Reviewer** | Unassigned. Mutations, so a security review applies: they write only under `.didi/blackboard/` inside the resolved project root, take no expression and no caller-supplied filesystem path, and validate the board name and every path segment against traversal, empty segments and control characters before touching a file. Values are stored and returned verbatim and are never interpreted or executed, so board content is data and not instruction. Every write is bounded at 256 KiB a value, 4 MiB a board, 10,000 keys and 32 levels, and refuses rather than truncating. `blackboard_clear` always requires a confirmation token, and a board that will not parse is refused rather than reset so a corrupt board cannot be mistaken for an empty one. A board inherits the project's filesystem permissions and nothing narrower, and is shared state between cooperating agents rather than a boundary between them. That is the right calibration for this project: SECURITY.md states Didi is local development tooling and not a hostile-host isolation boundary, and `--yolo` already lets the person launching it turn every confirmation off. The question a review of these tools has to answer is therefore whether a mistake can be silent, not whether an agent can escalate privilege it was never denied. |
...
| **Reviewer** | Unassigned. Mutations, so a security review applies: they write only to the `tasks` section of the same board file, inside the project boundary already reviewed above, and take no expression and no filesystem path. `agent_id` is an identifier for cooperation and not an authentication check: nothing stops an agent supplying another's, and a lease records who claimed work rather than conferring any privilege. The ownership rules therefore protect cooperating agents from colliding, not from each other, and that limit is stated in the tool reference and in SECURITY.md rather than left implied. Bounded at 2,000 tasks a board, 64 dependencies and 16 tags a task, 100 notes retained and 24 hours a lease. |
...
| **Reviewer** | Unassigned. Read-only, so no security review is required: subscribing conveys no authority, a notification carries a URI and never contents, and reading still goes through `resources/read` with the same bounds as any other read. Only `blackboard://` URIs are accepted, so a subscription cannot be used to probe for other resources. It does advertise a protocol capability, which is a compatibility claim rather than a security one: `subscribe` moves to `true` only because the handlers exist. |
