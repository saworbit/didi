...
| **Project** | Launch arguments, `project.godot`, `addons/didi`, and with an editor attached, the editor's unsaved-scene list | Green (`Ready`): root resolved, `addons/didi/didi.gdextension` present, and `project.godot` lists the plugin under `editor_plugins/enabled`. Amber: root no longer readable, no addon in this project, addon present but not enabled, or, with an editor attached on Godot 4.7 or later, an open scene holding unsaved changes (`Unsaved scene changes`, naming the scenes, with `editor_save_scene` as the fix). Each has its own fix. The addon checks are stats, so they answer in exactly the state where no live route can exist. The unsaved-scene read is the one live question the dashboard asks; `EditorInterface.get_unsaved_scenes` binds from Godot 4.7, so on 4.5 and 4.6 the `Unsaved scenes` fact says `not reported before Godot 4.7` and the light does not guess. |
