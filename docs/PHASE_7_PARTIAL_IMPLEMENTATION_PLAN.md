...
`signal_connect.flags` has default `2` and enum `[2,3,6,7,34,35,38,39]` since #852. `CONNECT_PERSIST` is required, `CONNECT_DEFERRED` and `CONNECT_ONE_SHOT` may be combined with it, and `CONNECT_INHERITED` is accepted and masked off because that is how `signal_list_connections` reports a connection inside an instanced scene. `CONNECT_REFERENCE_COUNTED` is still rejected: exact reference counts cannot be reconstructed, and this surface answers `409` for a callable that is already connected, so the count can never rise above one here. `CONNECT_APPEND_SOURCE_OBJECT` is rejected because it appends the emitter to the arguments, so the method needs one more than the signal declares, and connect preconditions on the signal's own arity; measured on 4.7.2, a zero-argument signal never reaches a zero-argument method with that flag set. Rollback stores and restores the exact callable, bind array, and observed flag value before mutation, and an undo of a disconnect now restores the flags the connection had rather than `2`.

The narrowing to `[2]` was this partial delivery's own, and the reasons it recorded did not survive being measured. Deferred and one-shot were held back as unproven, and the reason that reached `docs/TOOL_REFERENCE.md` became a claim about the engine -- that no other value survives a scene save predictably -- which is false on 4.5.1, 4.6.2 and 4.7.2: every combination containing the persist bit round-trips a pack, a save and a load exactly, and nothing without it is written to the file at all. The original plan's `{2,3,6,7,10,11,14,15}` was closer to the engine than the narrowing that superseded it. The one-shot rollback concern is real and is answered by detection rather than by refusal: a one-shot that fires before the postcondition read leaves no observed connection, which is already `signal_postcondition_mismatch` with the rollback attempted and reported.
...
- `signal_connect` request is exact object requiring `emitter_node` 1..1024, `signal_name` 1..128, `target_node` 1..1024, `target_method` 1..128, with `flags` enum `[2,3,6,7,34,35,38,39]` default `2`. Success is `{connected:true,flags,undo_redo_registered:true,outcome:"completed",rollback:"undo_redo"}`, where `flags` is what was written. Existing exact Callable identity is `409`. Undo/redo restores the exact callable, binds, and the flags the connection had. `CONNECT_PERSIST` is required, `CONNECT_DEFERRED` and `CONNECT_ONE_SHOT` may be combined with it, and `CONNECT_INHERITED` is accepted and masked off because that is how `signal_list_connections` reports a connection inside an instanced scene; reference-counted and append-source-object connections are refused. This enum was `[2]` until #852, on the premise that no other value survived a scene save, which the engine disproves on 4.5.1, 4.6.2 and 4.7.2. Error contract: exact `signal_connect` row in the authoritative error table.
- `signal_disconnect` has the same four required strings and no `flags` property. It accepts any observed exact connection the scene file stores, meaning one whose flags include `CONNECT_PERSIST`; an observed connection without it is `409 unsupported_existing_connection_flags` and is not changed. Success is `{disconnected:true,flags,undo_redo_registered:true,outcome:"completed",rollback:"undo_redo"}`, where `flags` is what the removed connection had, which is what `signal_connect` needs to put it back. Zero or multiple exact matches is `409`. Error contract: exact `signal_disconnect` row in the authoritative error table.
