...
| :--- | :--- | :--- |
| `offline_fallback` (managed-only host) | `runtime_recovery_status`, `runtime_checkpoint`, `runtime_recover_editor`, `runtime_restore_checkpoint` | Requires paired managed startup flags. Host-side status, saved-file snapshots, one automatic owned-editor restart, and confirmed restore; ordinary attachment returns a disabled error. |
