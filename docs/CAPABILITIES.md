...
Do not infer availability from a tool name or description. Do not call a tool when `implemented` is false. A live-only tool with `currentMode: "unavailable"` requires Godot 4.7+ with the Didi addon enabled.

## Canonical tools

Didi v1.4.0 registers 78 canonical tool names. Sixty are implemented in at least one mode; 18 remain reserved and return an MCP tool error. Ten legacy names are registered separately, for exactly 88 `tools/list` entries.
...
- Godot 4.7 does not expose active-scene dirty state through GDExtension. `scene_close` therefore refuses by default and requires `discard_unsaved: true`; this conservative contract prevents silent loss.
