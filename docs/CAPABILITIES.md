...
- `editor_reload_project`, script patching, `project_rename_references`, and overwrite-enabled `resource_create`, `script_create`, visual-test-lab creation, `project_export`, and `gridmap_export_mesh_library` require the preview's 64-hex `confirmation_token`. Tokens expire after 120 seconds, are single-use, and reject tool, argument, project, mode, session, generation, expiry, and replay mismatches.
