...
| `live`, `offline_fallback` | `project_audit_assets` | The scan is always project files and `scan_source` says so. A connected editor checks `unresolved_uid` findings against `ResourceUID`: ones it resolves move to `engine_only_references` and stop counting as orphans, ones it does not know gain `confirmed_by_engine`. `execution_mode` reports whether an engine contributed; `uid_verification` reports the detail. |
