...
- `script_check_syntax` combines lightweight diagnostics with a Godot `--headless --check-only` run whenever a file path is supplied. A Godot that cannot be launched, or that runs and prints no version banner, is refused `503` with `code: "engine_unavailable"` rather than answered as a script with no errors. `source_text`-only checks do not invoke Godot and are unaffected.
