...
Didi v1.5.0 registers 96 canonical tool names. 93 are implemented in at least one mode; 3 remain reserved and return an MCP tool error. In other words, 93 canonical tools are implemented. Ten legacy names are registered separately, for exactly 106 `tools/list` entries.
