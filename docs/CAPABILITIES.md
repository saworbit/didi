...
| `offline_fallback` (local management) | `runtime_list_sessions`, `runtime_attach_session`, `runtime_detach_session`, `runtime_get_session` | Scans validated same-user descriptors and changes the selected route in the standalone MCP process. Public payloads use `execution_mode: "local_session_management"`; v1.3.0 requires explicit attach and never returns the private token. |
| `live` | `runtime_read_logs`, `runtime_set_paused`, `runtime_step`, `runtime_stop`, `runtime_get_tree`, `eval_gdscript` | Requires an explicitly attached editor/game session. Operations execute on that Godot process's main thread and identify `session_kind`; game-only control rejects editor sessions. |
...
- The default registry is `<OS temporary directory>/didi-sessions`; `DIDI_SESSION_DIR` is a test/controlled-environment override. Shutdown retires an owned descriptor to a non-`.json` no-replace tombstone before deletion; collision/exhaustion can intentionally retain an owner-only retired file rather than overwrite another path.
- Attach uses a 3,000 ms authenticated handshake and swaps routes only after it succeeds. A failed attach preserves the previous healthy route. `runtime_get_session` is a local metadata read; it does not perform a fresh handshake.
