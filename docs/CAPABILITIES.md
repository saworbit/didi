...
- `currentMode` is `live`, `offline_fallback`, `local`, `local_status`, `local_session_management`, `unavailable`, or `unimplemented` for the current process. It uses the same vocabulary as a result payload's `execution_mode`, and `executionModes` does too: a tool with no `live` path advertises and reports `local` rather than `offline_fallback`, whether or not a session is attached. `offline_fallback` means the call would have gone to the engine and could not, which is the only case where reattaching would improve the answer. `local_status` and `local_session_management` are the two more specific members of the local family. `execution_mode` remains a statement about the call that just ran, so a tool that does have a live path can report `local` for a call with no live work to do: `project_get_uid_map` with no `resolve`, and `project_audit_assets` with nothing an engine could verify.
...
Every tool definition also carries specification `annotations`. `readOnlyHint` is false for every tool that mutates the project, and for `runtime_attach_session` and `runtime_detach_session`, which change the attachment every later live call routes through. In managed mode, ordinary authorized reads may consume the single automatic editor restart and execute project startup code, so read-only auto-approval must account for that effect. `destructiveHint` is true for every writer that can replace something, and false for the writers that can only add: `scene_instantiate_node`, `scene_duplicate_node`, `scene_add_to_group`, `signal_connect`, `blackboard_task_create`, `runtime_checkpoint`, `instantiate_asset` and `mutate_scene_tree`. A writer that takes an `overwrite` flag can replace a file, so it stays destructive. `idempotentHint` is true for the writers that land in the same state when the same call is made twice, which is what a client reads to decide whether a call that timed out can be sent again. `openWorldHint` is true for `runtime_recover_editor`, `runtime_restore_checkpoint`, `csharp_check_build`, `shader_check_compile`, `project_export`, `gridmap_export_mesh_library`, `runtime_launch`, and `script_check_syntax`, and false for other tools. These can launch project code, including scripts, extensions, export plugins, package restoration, or custom build targets, with the local account's access. A local working directory is not an OS sandbox. Successful JSON results additionally carry `structuredContent` holding the same payload as the text block after execution-mode and session attribution.

Do not infer availability from a tool name or description. Do not call a tool when `implemented` is false. A live-only tool with `currentMode: "unavailable"` requires Godot 4.5+ with the Didi addon enabled. Which of those is missing is a question for the person at the editor, not for the client: the addon's **Didi** main screen tab distinguishes an extension that is not loaded from one that is loaded without a published session, and names the file, path or pid behind each.

## Canonical tools

The current source / Unreleased surface registers 116 canonical tool names. 113 are implemented in at least one mode; 3 remain reserved and return an MCP tool error. In other words, 113 canonical tools are implemented. Ten legacy names are registered separately, for exactly 126 `tools/list` entries. The latest documented release is 2.0.0; its historical surface is recorded in the changelog.

| Execution modes | Canonical tools | Current behavior |
| :--- | :--- | :--- |
| `local` (managed-only host) | `runtime_recovery_status`, `runtime_checkpoint`, `runtime_recover_editor`, `runtime_restore_checkpoint` | Requires paired managed startup flags. Host-side status, saved-file snapshots, one automatic owned-editor restart, and confirmed restore; ordinary attachment returns a disabled error. |
| `live`, `offline_fallback` | `scene_get_hierarchy`, `viewport_capture_frame` | Uses the edited SceneTree, the real editor viewport, or a game's root viewport when connected; otherwise parses a `.tscn` file or returns an explicitly synthesized grid PNG. |
...
| `local_session_management` | `runtime_list_sessions`, `runtime_attach_session`, `runtime_detach_session`, `runtime_get_session` | Scans validated access-controlled descriptors and changes the selected route in the standalone MCP process. Public payloads use `execution_mode: "local_session_management"`, which is also what `tools/list` advertises, and never return the private token. |
...
| `live`, `offline_fallback` | `project_audit_assets` | The scan is always project files and `scan_source` says so. A connected editor checks both kinds of broken reference: `unresolved_uid` against `ResourceUID` and `missing_file` against `ResourceLoader.exists`. Findings it disproves move to `engine_only_references` and stop counting as orphans; findings it agrees with gain `confirmed_by_engine`. `execution_mode` reports whether an engine contributed; `reference_verification` reports the detail. |
...
| `local` | `script_check_syntax`, `script_reflect_class`, `script_get_symbols`, `script_patch_method`, `script_create`, `viewport_create_test_lab`, `resource_create`, `resource_inspect`, `project_list_resources`, `project_analyze_impact`, `project_verify_changes`, `project_apply_changes`, `project_rename_references`, `blackboard_write`, `blackboard_read`, `blackboard_patch`, `blackboard_list_keys`, `blackboard_clear`, `blackboard_task_create`, `blackboard_task_claim`, `blackboard_task_update`, `blackboard_task_complete`, `blackboard_task_list`, `project_search_text`, `project_search_symbols`, `runtime_launch`, `csharp_check_build`, `shader_check_compile`, `project_list_export_presets`, `project_export`, `gridmap_export_mesh_library` | Operates on bounded project files or launches a separate Godot/dotnet process. Results are not live editor state. |
| `live`, `offline_fallback`, `local` | `project_get_uid_map`, `project_audit_assets` | Whether an engine contributes depends on the arguments rather than on the route. `project_get_uid_map` resolves `uid://` and `res://` queries against the engine's `ResourceUID` table; the map itself is a file scan in every mode, because `ResourceUID` exposes no enumeration through GDExtension. `project_audit_assets` has an attached editor verify its unresolved-uid and missing-path findings and clear the ones it disproves. A call with no live work to do -- a map with no `resolve`, a scan that found nothing an engine could check -- answers `local`, not `offline_fallback`: there is nothing to fall back from and reattaching would change nothing. |
| `local_status` | `didi_control_room` | Reports this server's own bridge, surface, safety and log state. Reads no Godot and writes nothing. Its tool rows come from the same availability function as `tools/list`, so the two cannot disagree. |
| `unimplemented` | `physics_simulate_step`, `nav_bake_mesh`, `runtime_get_call_stack` | Registered schema only. Calls are rejected before legacy handlers execute. Each has a stand-in that is not the same thing and is documented as not the same thing: `runtime_set_paused` plus `runtime_step` advances a game by whole frames rather than by exact physics ticks at a caller delta; a region baked in the editor or in project GDScript is what `nav_query_path` then queries; `runtime_read_output` names the file, function and line an error was raised in, which is one frame and not a stack. See [Tool Reference](TOOL_REFERENCE.md#reserved-physics-and-navigation-schemas--unimplemented). |
...
| `analyze_script_diagnostics` | `script_check_syntax` | `local` |
| `patch_script_symbols` | `script_patch_method` | `local` |
| `create_visual_test_lab` | `viewport_create_test_lab` | `local` |
| `query_project_resources` | `project_list_resources` | `local` |
| `execute_test_session` | `runtime_launch` | `local` |
...
| :--- | :--- | :--- |
| `godot://project/tree` | `local` | Filesystem/resource-index snapshot rooted at the explicit canonical Godot project. |
...
| `blackboard://<board>/state` | `local` | Shared state on a board, with the author, reason and expiry recorded per path. The default board is listed and `resources/templates/list` publishes the parameterised shape; any other board resolves on demand and carries `exists` so an unwritten board is distinguishable from an empty one. Served as `application/json` whichever board it is. Subscribable. |
| `blackboard://<board>/tasks` | `local` | Tasks on a board with status, lease and dependencies. Lapsed leases are reclaimed before the answer is built. Carries `exists` and the same parameterised shape as the state resource. Subscribable. |
...
- The [always-confirmed and overwrite-confirmed tools](TOOL_REFERENCE.md#13-phase-6-mutation-safety) require the preview's 64-hex `confirmation_token`. Tokens expire after 120 seconds, are single-use, and reject tool, argument, project, mode, session, generation, expiry, and replay mismatches.
...
- Board bounds: 256 KiB a value, 4 MiB a board, 10,000 keys, 32 levels of nesting, 32 path segments, 100 operations a patch, and 30 days on `ttl_seconds`. A write that would breach one is refused and nothing is written. `blackboard_patch` is all or nothing.
...
- `resources/subscribe` accepts `blackboard://` URIs only, because those are the resources Didi publishes `notifications/resources/updated` for. Accepting a subscription to anything else would promise updates that never arrive. This is a limit of Didi rather than a fact about the resources: the three `godot://` resources do change on their own, `godot://runtime/logs` most obviously, and until Didi publishes notifications for them they have to be polled through their own tools. `initialize` advertises `resources.subscribe` for the server as a whole, so each `resources/list` entry carries `_meta.didi.subscribable` for the part that applies to it.
...
- `script_check_syntax` combines lightweight diagnostics with a Godot `--headless --check-only` run whenever a file path is supplied. An executable that exists and prints no Godot version banner is refused `503` with `code: "engine_unavailable"` rather than answered as a script with no errors; a machine with no Godot installed is a different state and still answers, with `engine_available: false` saying the compiler did not run. `source_text`-only checks do not invoke Godot and are unaffected.
...
- `project_verify_changes` checks a proposal in an isolated git worktree so a script that preloads a sibling sees the proposed sibling, and nothing reaches the working tree whether the proposal is good or not. It needs the project to be inside a git work tree, carries uncommitted work across, and names untracked files it could not carry. `run_scene` opens a scene there as well, headless and bounded by `run_frames`, so a scene that fails to load is caught rather than a file that merely parses. The copy has no import cache, so a first run also imports what the scene touches, out of the same timeout. No frames are captured; a headless run has no renderer.
- `project_apply_changes` runs that same check and, only if it passes, writes the proposal into the working tree in one staged pass. A proposal that fails writes nothing and comes back as the verification report. It always requires a confirmation token, because it can replace several existing files at once with no undo stack behind them.
- `project_rename_references` rewrites only the forms Godot serializes: `[connection]` signal and method attributes, and the property segment of an animation track `NodePath`. GDScript and C# references are reported and never rewritten, because a whole-word match in a dynamically typed language may be an unrelated local, and the `[autoload]` key in `project.godot` is reported the same way and for the same reason. It refuses a colliding `new_name` and refuses to run on a truncated scan.
- `scene_set_property` and the `properties` argument of `scene_instantiate_node` accept nil, bool, int, float, String, StringName, NodePath, Vector2/Vector2i/Vector3/Vector3i from `{x,y[,z]}` objects, Color from `{r,g,b[,a]}` or a hex string, and Resource slots from a `res://` path validated against the classes the property takes, which Godot declares as a comma-separated list and which may name exclusions. Other Godot types, Rect2 and Transform among them, are still refused rather than guessed at.
- `viewport_create_test_lab` writes a sandbox `.tscn` at the project root; it preserves an existing sandbox unless `overwrite: true` is explicit, instances a PackedScene target and attaches any other resource to a holder node (reported as `target_instanced`), and does not produce multi-angle live captures automatically.
...
- Offline scene and script file reads stay beneath the project working directory, including UTF-8 paths on Windows; they never fall back to `demo/` or recursively guess a scene. Resource UID discovery supports bounded, format-validated Godot 4.3+ `.uid` sidecars for all indexed resource types, and reads the `uid` line out of the `.import` metadata Godot writes for imported assets, which get no `.uid` sidecar of their own.
- Godot 4.5 and 4.6 do not expose active-scene dirty state through GDExtension; `EditorInterface` there offers only the write-side `mark_scene_as_unsaved`. Godot 4.7 adds the read-side `EditorInterface.get_unsaved_scenes()`, which `scene_close` consults through a runtime bind probe rather than a version string. Where that bind is absent, or the active scene has never been saved and so has no path the engine can name, `scene_close` still refuses by default and requires `discard_unsaved: true`. Where the bind is present and the engine positively omits the active scene from its unsaved list, the default call closes and reports `dirty_state: "clean"`. A scene the engine names as unsaved is refused on every version. Every result carries `dirty_state_readable` so a client can tell a checked close from an unchecked one.
...
- `audio_configure_bus` writes the running engine's `AudioServer` and no file of its own. In an attached editor the change still reaches the project: the editor's own bus-layout autosave picks it up and writes `res://default_bus_layout.tres` a moment later. The result says which case it is with `persisted_by_editor`, `layout_path` and a `limitation`, because `undo_redo_registered: false` beside `revert_with` otherwise reads as "nothing was committed".
- `shader_set_uniform` sets one uniform through the editor UndoRedo stack, taking the same JSON spellings `scene_set_property` takes, and refuses a name the shader does not declare or a value outside a declared `hint_range`. The Variant is built for the type the shader declares, and the result says the change is not on disk yet. The write reaches the loaded material: an embedded one is saved with the scene, an external `.tres` is not saved by this surface, and a shared material is shared.
- `shader_list_uniforms` reads a ShaderMaterial's uniforms from a node in the edited scene, with each declared type, the hint the shader author wrote, its effective value, and whether the property contract can express that type. The effective value is the material's override or the shader's declared default, read from the rendering server when the material does not set it, and the two are not distinguished. A null value means neither could supply one, which is what a session with no renderer looks like.
- `editor_render_ghost_preview` and `editor_clear_ghost_previews` draw wireframe boxes in the editor viewport for a mutation nobody has made yet, in 2D or 3D, with cyan for an addition, yellow for a translation and red for a deletion. The shapes go to the rendering server rather than into the scene, so nothing is added to the tree and the editor never becomes dirty. They stay up until cleared or the editor closes.
- `viewport_capture_passes` redraws the live 3D scene with replacement materials and returns depth and world-space normal images beside the ordinary colour frame. The values are orderings rather than measurements: the viewport post-processes after the pass shader writes, and by how much depends on the engine version. Only `GeometryInstance3D` nodes are painted, the viewport's own background stays behind them, and every `material_override` is restored before the call returns. `segmentation` paints each node its own flat colour and returns a legend of node paths, observed colours, pixel counts and 2D bounds. The legend reports the colour that came back rather than the one the shader was given, so it matches its own pixels on an engine that shifts them and on one that does not. Nodes past the palette are named rather than sharing a colour.
