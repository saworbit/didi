...
- **B3 — `runtime_inject_input` (Phase 7C).** Ship as a bounded batch of `key | mouse_button | mouse_motion | action | wait | step | click_control`, not single events. `click_control` resolves through `ui_list_controls` to turn a name into a rectangle, and `ui_hit_test` to confirm what actually sits at the point it picks.
- **B4 — `ui_list_controls`. Delivered.** Bounded list of Controls with rect, class, visibility, mouse filter and text, in an editor or a running game. Makes B3's `click_control` addressable by name. See [Surface Amendments](SURFACE_AMENDMENTS.md).
