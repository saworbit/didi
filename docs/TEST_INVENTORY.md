...
**803 tests.** Derived from `didi_tests --list`, which prints the registry the runner iterates.
...
**406 tests.** Derived from `test_*` methods on every `unittest.TestCase` subclass, read with `ast`.
