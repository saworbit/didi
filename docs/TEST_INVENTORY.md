...
**801 tests.** Derived from `didi_tests --list`, which prints the registry the runner iterates.
...
**402 tests.** Derived from `test_*` methods on every `unittest.TestCase` subclass, read with `ast`.
