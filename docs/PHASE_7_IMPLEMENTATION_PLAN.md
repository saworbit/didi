...
**Goal:** Implement the remaining 18 registered canonical tools with native Godot-derived behavior so Didi moves atomically from 61/79 implemented to 82/82 without changing the 79 canonical names or 10 compatibility aliases.
...
- **B)** Retain atomic 82/82 and wait for supported engine capabilities.
...
Expected: both exit `0`; all current documents agree on 82/82, 0 remaining, 10 legacy, and Phase 7 complete.
