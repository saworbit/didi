...
AI agents do not work at human speed. A useful next shape is a **lab**: the standalone `didi` process stays the MCP tool host, and behind it it can own several short-lived Godot processes so an agent can say "give me twenty benches and try this in parallel."

The lab is an orchestrator, not an agent. Planning, retry policy, and "is this level good" stay in the LLM client. Didi supplies isolated Godot compute, honest routing, and bounded results.

This is complementary to MCP, not a replacement. MCP remains the agent-to-tool surface. A2A, if added later, would only advertise the lab as a peer. It would not replace stdio MCP or the local IPC session protocol.
...
| Term | Meaning |
| :--- | :--- |
| **MCP host** | The standalone `didi` / `didi.exe` process. One per IDE/agent connection. |
...
| **Bench** | A Godot process **owned by this MCP host**: spawned, authenticated, leased, reaped. Usually `kind=game`, headless. |
| **Lab** | The pool of benches plus the ledger of experiments this host currently owns. |
...
A user-opened editor remains a **session**. It is attachable. It is not a bench unless this host spawned it. Mixing those two ownership models is how you corrupt a human's unsaved scene.

---

## 3. What we will not build

These are hard non-goals for the first lab milestone. Several already appear in `docs/ROADMAP.md`.
...
- Do not treat `runtime_launch` as the lab. That tool waits for the child to **exit** (1–120 s) and returns stdout/stderr. A bench must stay alive and publish a descriptor.
- Do not silently write into `demo/` or Didi's own repository CWD. Lab workspaces are explicit directories.
- Do not raise the 72-tool count by stuffing stubs. New names land only when they execute.
...
| Stop | `runtime_stop` on an attached **game** | Stop/reap a bench the host spawned, including a wedged process |
...
1. **Foreign sessions are read-only from the lab's point of view.** A human editor the user already opened can be listed and attached through the existing tools. The lab must not reap it, overwrite its project, or use it as a parallel mutation worker.
2. **One editor bench per project per host, default zero.** Editors are for the human. Parallel labs use headless games.
...
4. **Workspaces live outside the source tree.** Suggested default: `<project>/.didi-lab/<bench-id>/` is **wrong** if that dirties version control. Prefer `<OS temp>/didi-lab/<host-id>/<bench-id>/` or an explicit `--lab-root`. The host records the mapping. `.didi-lab/` under the project is allowed only when gitignored and documented.
...
6. **Reap is the host's job.** On MCP shutdown, bench TTL expiry, explicit release, or authenticated identity failure, the host sends `runtime.stop` when possible, then terminates the child process group, then retires only an exact identity-matched descriptor using the existing proof-safe path.
...
Do not register these until the implementation phase. Keep the 72-tool count stable until then.

Suggested canonical names, Domain 13:
...
An optional `session_id` / `bench_id` argument on live tools is attractive and dangerous. It changes every live handler. Defer it until `lab_run` proves routing. Until then, the user's active route stays the only implicit target for `scene_*`, `viewport_*`, and `eval_gdscript`.
...
The lab controller stores the child process handle (Windows) or process group (POSIX) so reap does not depend on the descriptor still being present.

---

## 10. Experiment outcomes

Every `lab_run` item returns the same honesty flags the rest of Didi already uses:
...
`lab_run` itself is success at the MCP layer when the request was valid and every bench was addressed. Partial Godot failure lives in `results[]`. That matches Phase 3: transport and tool errors are explicit, not hidden behind a fake top-level success with empty data.
...
This is the loop the conversation was actually about.

1. Agent calls `lab_acquire_benches` with `count: 4`, `kind: game`, a scene, `isolate_workspace: true`.
2. Agent calls `lab_run` with `viewport_capture_frame` or a later input/physics tool once those exist.
...
Until `runtime_inject_input` and physics tools are real, the honest lab loops are:
...
- The MCP host, not the GDExtension, presents the Agent Card.
- Skills map onto `lab_acquire_benches` / `lab_run` / `lab_release_benches`.
...
The lab inherits the local attachment boundary and adds process-control risk.
...
- Workspace paths are canonicalized and must stay under `--lab-root` or the platform temp lab directory.
...
- Do not run lab benches as Administrator / root because an editor happened to. Prefer the MCP host's own uid.
- Capture IDs and RGBA caches stay inside the child. The host must not merge caches across benches.
- `eval_gdscript` stays the read-only expression subset. A lab does not relax the sandbox because the process is "just a worker."
...
New internal type `OwnedBench` in the standalone process. Extend `TestRunner` or add `LabSpawner` so a child can be started **without** waiting for exit. Wait for descriptor + handshake. Add `lab_status` against owned state only. Native tests with a fake descriptor publisher; no Godot required yet.
...
### Slice D — Public lab tools

`lab_acquire_benches` / `lab_release_benches` / `lab_run`. Documentation, capability metadata, changelog, tool count bump in the same change. Godot integration on 4.5.1 at least two headless benches.
...
- Foreign session ids are refused by release and `lab_run`.
...
- `lab_run` `runtime_get_tree` returns both trees.
...
Until then this file is the only new contract. It does not change version `1.4.0` or the 72 / 10 / 82 / 54 / 18 release facts.
...
| Share the user's editor? | List/attach only. Never lab-reap. |
