...
- To give an `AnimationPlayer` an animation, write the `Animation` and an `AnimationLibrary` whose `_data` maps a name to it with `resource_create`, then call `anim_add_library`. Use the names in its `animations` field with `anim_play_track`: a named library's animations are `library/animation`, and the default library's (`library_name` left empty) are their own names. Save the scene before launching the game, because `anim_play_track` only plays in a running game.
