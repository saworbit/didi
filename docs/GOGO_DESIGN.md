...
**Naming:** Didi remains the product, binary (`didi` / `didi.exe`), GDExtension, and MCP server. Gogo is the subsystem. Code lives under `didi::gogo`. Do not rename the repository or the existing canonical surface.
...
- Do not raise the canonical count by stuffing stubs. New names land only when they execute, and only through an accepted amendment in [Surface Amendments](SURFACE_AMENDMENTS.md).
...
Do not register these until the implementation phase, and then only through an accepted amendment in [Surface Amendments](SURFACE_AMENDMENTS.md). Nothing here is a reason to register a name that does not execute.
