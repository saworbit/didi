...
**Naming:** Didi remains the product, binary (`didi` / `didi.exe`), GDExtension, and MCP server. Gogo is the subsystem. Code lives under `didi::gogo`. Do not rename the repository or the existing 72-tool surface.
