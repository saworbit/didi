...
Godot 4.5 and 4.6 cannot report editor dirty state through GDExtension, so `scene_close` requires `discard_unsaved: true` there. That is a conservative data-loss guard, not a success stub. Godot 4.7 adds `EditorInterface.get_unsaved_scenes()`, and `scene_close` now consults it behind a runtime bind probe: a scene the engine positively reports as clean closes without the flag, and every other case keeps the guard. See [Surface Amendments](SURFACE_AMENDMENTS.md).
...
- Atomic schema-1 descriptors in the per-user platform registry (Windows temp child; POSIX XDG runtime child or euid-suffixed temp fallback), process-unique same-user endpoints, PID plus process-start identity, private 64-hex tokens, and a finite protocol-1.3 handshake deadline.
...
**Objective:** The implementation remains 115/118 canonical tools, and all 3 Phase 7 names remain registered but unimplemented. All 15 feasible names are delivered. The original objective was atomic 83/83 without adding public tool names.
...
authoritative at 115/118. Phase 7 remains `PARTIAL_DELIVERY` because the three
