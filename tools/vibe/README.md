...
| `probes/localised_controls.py` | `localisation_engine.py`'s first finding asked of the surface: a menu whose `Label` and `Button` hold the key `MENU_START`, run as a detached headless game twice, with `ui_list_controls`' `text` beside the game's own report of what each control displays (`atr(text)`, read back through `runtime_read_output`). The control, with nothing registered, must agree; with the `.translation` files registered and `locale/test` set to `fr` through `project_set_setting`, the tool answers the key while the game displays "Commencer la partie". Writes the menu and CSV into a `sandbox.py --build-tree` project, imports them and opens and stops its own headless editor. Pass `--godot` for the engine line. |
