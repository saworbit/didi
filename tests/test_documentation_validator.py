...
# The four signal names and the profiler shipped; the other thirteen Phase 7 names are reserved.
...
DELIVERED_TOOL_NAMES = VALIDATOR.PHASE7_CANONICAL_TOOLS[:4] + VALIDATOR.PHASE7_CANONICAL_TOOLS[-1:]
RESERVED_TOOL_NAMES = VALIDATOR.PHASE7_CANONICAL_TOOLS[4:-1]
...
Phase 7 status is PARTIAL_DELIVERY after the 2026-08-29 Godot 4.5.1 and Godot 4.7.2 feasibility gate. The implementation remains 70/83 canonical tools, with all 13 Phase 7 names registered but unimplemented. The gate found 15/18 implementation-feasible and 3/18 API-blocked under the approved contracts.
...
Discovery now exposes 83 canonical tools plus 10 legacy registrations (93 total). 70 canonical tools are implemented and 13 remain unimplemented.
...
""",
        ...
        generic_docs = {
            ...
            "DEVELOPER_GUIDE.md": "# Developer Guide\n\nPhase 7 is PARTIAL_DELIVERY at 70/83 implemented; 15/18 names are implementation-feasible and 3/18 are API-blocked.\n",
            "INTEGRATION_GUIDE.md": "# Integration Guide\n",
            "LLM_INSTRUCTIONS.md": "# LLM Instructions\n\nStart with --project or DIDI_PROJECT_ROOT. Preview with dry_run and use confirmation_token when returned. Phase 7 is PARTIAL_DELIVERY at 70/83 implemented; all 13 names remain unimplemented, including the 15/18 implementation-feasible names and the 3/18 API-blocked names.\n",
...
Didi v1.4.0 registers 83 canonical tool names. 70 are implemented in at least one mode; 13 remain reserved. Ten legacy names are registered separately, for exactly 93 tools/list entries.

Phase 7 is PARTIAL_DELIVERY at 15/18 implementation-feasible and 3/18 API-blocked; feasibility is not implementation and all 13 remain unimplemented.
...
Phase 7 is PARTIAL_DELIVERY at 70/83 implemented. All 13 Phase 7 names remain unimplemented; 15/18 are implementation-feasible and 3/18 are API-blocked under the approved contracts.
    ...
    def make_future_phase_roadmap(
        ...
        for phase, name in phase_names.items():
            ...
            if phase == 7:
                lines.extend(
                    [
                        "**Objective:** The implementation remains 70/83 canonical tools, with all 13 Phase 7 names registered but unimplemented. The feasibility gate found 15/18 implementation-feasible and 3/18 API-blocked under the approved contracts.",
                        "",
                    ]
                )
    ...
    def make_future_phase_governance(
        ...
        for phase in range(7, 13):
            ...
            if phase == 7:
                lines.extend(
                    [
                        "**Goal:** The implementation remains 70/83 canonical tools, with all 13 Phase 7 names registered but unimplemented; 15/18 are implementation-feasible and 3/18 are API-blocked under the approved contracts.",
                        "",
                    ]
                )
...
"""
    ...
    def write_manifest(self, **overrides) -> Path:
        ...
        counts = {
            ...
            "implemented": 70, "unimplemented": 13, "total": 93,
        }
    ...
    def test_manifest_disagreeing_with_documentation_fails(self):
        ...
        errors = VALIDATOR.validate_repository(
            root, self.write_manifest(implemented=71, unimplemented=12)
        )
        self.assertTrue(
            any("must state 71 implemented tools" in e for e in errors), errors
        )
    ...
    def test_manifest_drives_the_canonical_count_check(self):
        ...
        self.assertTrue(
            any("must be 83 canonical, 71 implemented, and 12 remaining" in e for e in errors),
            errors,
        )
    ...
    def test_stale_fallback_counts_are_reported(self):
        ...
        root = self.make_valid_repository()
        errors = VALIDATOR.validate_repository(
            root, self.write_manifest(canonical=84, implemented=71, total=94)
        )
        self.assertTrue(
...
"""
    ...
    def test_rejects_canonical_count_mutation_in_each_current_document(self):
        mutations = {
            "README.md": ("all 13 Phase 7 names", "all 12 Phase 7 names"),
            "docs/CAPABILITIES.md": ("13 remain reserved", "12 remain reserved"),
            "docs/ROADMAP.md": ("all 13 Phase 7 names", "all 12 Phase 7 names"),
            "docs/FUTURE_PHASES_DESIGN.md": (
                "all 13 Phase 7 names",
                "all 12 Phase 7 names",
            ),
            "CHANGELOG.md": ("13 remain unimplemented", "12 remain unimplemented"),
        }
    ...
    def test_enforces_canonical_count_arithmetic(self):
        ...
        self.write(
            "docs/ROADMAP.md",
            roadmap.replace("all 13 Phase 7 names", "all 12 Phase 7 names", 1),
        )
