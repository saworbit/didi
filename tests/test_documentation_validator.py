...
# The four signal names, the profiler and input injection shipped; the other twelve Phase 7 names are reserved.
...
DELIVERED_TOOL_NAMES = VALIDATOR.PHASE7_CANONICAL_TOOLS[:4] + VALIDATOR.PHASE7_CANONICAL_TOOLS[-3:-2] + VALIDATOR.PHASE7_CANONICAL_TOOLS[-1:]
RESERVED_TOOL_NAMES = VALIDATOR.PHASE7_CANONICAL_TOOLS[4:-3] + VALIDATOR.PHASE7_CANONICAL_TOOLS[-2:-1]
...
Didi exposes 83 canonical tools plus 10 legacy names (93 total).

Startup requires --project or DIDI_PROJECT_ROOT. Mutations expose dry_run and protected writes use confirmation_token.

Phase 7 status is PARTIAL_DELIVERY after the 2026-08-29 Godot 4.5.1 and Godot 4.7.2 feasibility gate. The implementation remains 71/83 canonical tools, with all 12 Phase 7 names registered but unimplemented. The gate found 15/18 implementation-feasible and 3/18 API-blocked under the approved contracts.
...
Discovery now exposes 83 canonical tools plus 10 legacy registrations (93 total). 71 canonical tools are implemented and 12 remain unimplemented.
...
""",
        ...
        generic_docs = {
            ...
            "DEVELOPER_GUIDE.md": "# Developer Guide\n\nPhase 7 is PARTIAL_DELIVERY at 71/83 implemented; 15/18 names are implementation-feasible and 3/18 are API-blocked.\n",
            "INTEGRATION_GUIDE.md": "# Integration Guide\n",
            "LLM_INSTRUCTIONS.md": "# LLM Instructions\n\nStart with --project or DIDI_PROJECT_ROOT. Preview with dry_run and use confirmation_token when returned. Phase 7 is PARTIAL_DELIVERY at 71/83 implemented; all 12 names remain unimplemented, including the 15/18 implementation-feasible names and the 3/18 API-blocked names.\n",
...
Didi v1.4.0 registers 83 canonical tool names. 71 are implemented in at least one mode; 12 remain reserved. Ten legacy names are registered separately, for exactly 93 tools/list entries.

Phase 7 is PARTIAL_DELIVERY at 15/18 implementation-feasible and 3/18 API-blocked; feasibility is not implementation and all 12 remain unimplemented.
...
Phase 7 is PARTIAL_DELIVERY at 71/83 implemented. All 12 Phase 7 names remain unimplemented; 15/18 are implementation-feasible and 3/18 are API-blocked under the approved contracts.
...
Task 1 completed on 2026-08-29 with 15/18 implementation-feasible and 3/18 API-blocked. The all-or-nothing 83/83 gate originally prevented Tasks 2-13 and was replaced by an explicit partial-delivery decision. See [reproducible evidence](PHASE_7_API_FEASIBILITY.md).
""",
        )
        self.write(
            "docs/GUIDE.md",
    ...
    def make_future_phase_roadmap(
        ...
        for phase, name in phase_names.items():
            ...
            if phase == 7:
                lines.extend(
                    [
                        "**Objective:** The implementation remains 71/83 canonical tools, with all 12 Phase 7 names registered but unimplemented. The feasibility gate found 15/18 implementation-feasible and 3/18 API-blocked under the approved contracts.",
                        "",
                    ]
                )
    ...
    def make_future_phase_governance(
        ...
        for phase in range(7, 13):
            ...
            if phase == 7:
                lines.extend(
                    [
                        "**Goal:** The implementation remains 71/83 canonical tools, with all 12 Phase 7 names registered but unimplemented; 15/18 are implementation-feasible and 3/18 are API-blocked under the approved contracts.",
                        "",
                    ]
                )
...
"""
    ...
    def write_manifest(self, **overrides) -> Path:
        ...
        counts = {
            ...
            "implemented": 71, "unimplemented": 12, "total": 93,
        }
    ...
    def test_manifest_disagreeing_with_documentation_fails(self):
        ...
        errors = VALIDATOR.validate_repository(
            root, self.write_manifest(implemented=72, unimplemented=11)
        )
        self.assertTrue(
            any("must state 72 implemented tools" in e for e in errors), errors
        )
    ...
    def test_manifest_drives_the_canonical_count_check(self):
        ...
        self.assertTrue(
            any("must be 83 canonical, 72 implemented, and 11 remaining" in e for e in errors),
            errors,
        )
    ...
    def test_stale_fallback_counts_are_reported(self):
        ...
        root = self.make_valid_repository()
        errors = VALIDATOR.validate_repository(
            root, self.write_manifest(canonical=84, implemented=72, total=94)
        )
        self.assertTrue(
...
""",
    ...
    def test_reports_missing_current_release_fact(self):
        ...
        self.write("README.md", readme.replace("83 canonical", "77 canonical"))

        errors = VALIDATOR.validate_repository(root)

        self.assertTrue(any("README.md" in error and "83 canonical" in error for error in errors), errors)
...
"""
    ...
    def test_rejects_canonical_count_mutation_in_each_current_document(self):
        mutations = {
            "README.md": ("all 12 Phase 7 names", "all 11 Phase 7 names"),
            "docs/CAPABILITIES.md": ("12 remain reserved", "11 remain reserved"),
            "docs/ROADMAP.md": ("all 12 Phase 7 names", "all 11 Phase 7 names"),
            "docs/FUTURE_PHASES_DESIGN.md": (
                "all 12 Phase 7 names",
                "all 11 Phase 7 names",
            ),
            "CHANGELOG.md": ("12 remain unimplemented", "11 remain unimplemented"),
        }
    ...
    def test_enforces_canonical_count_arithmetic(self):
        ...
        self.write(
            "docs/ROADMAP.md",
            roadmap.replace("all 12 Phase 7 names", "all 11 Phase 7 names", 1),
        )
