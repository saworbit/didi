...
Didi exposes 81 canonical tools plus 10 legacy names (91 total).

Startup requires --project or DIDI_PROJECT_ROOT. Mutations expose dry_run and protected writes use confirmation_token.

Phase 7 status is PARTIAL_DELIVERY after the 2026-08-29 Godot 4.5.1 and Godot 4.7.2 feasibility gate. The implementation remains 67/81 canonical tools, with all 14 Phase 7 names registered but unimplemented. The gate found 15/18 implementation-feasible and 3/18 API-blocked under the approved contracts.
...
Discovery now exposes 81 canonical tools plus 10 legacy registrations (91 total). Sixty-seven canonical tools are implemented and 14 remain unimplemented.
...
""",
        ...
        generic_docs = {
            ...
            "DEVELOPER_GUIDE.md": "# Developer Guide\n\nPhase 7 is PARTIAL_DELIVERY at 67/81 implemented; 15/18 names are implementation-feasible and 3/18 are API-blocked.\n",
            "INTEGRATION_GUIDE.md": "# Integration Guide\n",
            "LLM_INSTRUCTIONS.md": "# LLM Instructions\n\nStart with --project or DIDI_PROJECT_ROOT. Preview with dry_run and use confirmation_token when returned. Phase 7 is PARTIAL_DELIVERY at 67/81 implemented; all 14 names remain unimplemented, including the 15/18 implementation-feasible names and the 3/18 API-blocked names.\n",
...
Didi v1.4.0 registers 81 canonical tool names. Sixty-seven are implemented in at least one mode; 14 remain reserved. Ten legacy names are registered separately, for exactly 91 tools/list entries.
...
Didi exposes 81 canonical tool names plus 10 legacy names (91 registrations).

Phase 7 is PARTIAL_DELIVERY at 67/81 implemented. All 14 Phase 7 names remain unimplemented; 15/18 are implementation-feasible and 3/18 are API-blocked under the approved contracts.
...
Task 1 completed on 2026-08-29 with 15/18 implementation-feasible and 3/18 API-blocked. The all-or-nothing 81/81 gate originally prevented Tasks 2-13 and was replaced by an explicit partial-delivery decision. See [reproducible evidence](PHASE_7_API_FEASIBILITY.md).
""",
        )
        self.write(
            "docs/GUIDE.md",
    ...
    def make_future_phase_roadmap(
        ...
        for phase, name in phase_names.items():
            ...
            if phase == 7:
                lines.extend(
                    [
                        "**Objective:** The implementation remains 67/81 canonical tools, with all 14 Phase 7 names registered but unimplemented. The feasibility gate found 15/18 implementation-feasible and 3/18 API-blocked under the approved contracts.",
                        "",
                    ]
                )
    ...
    def make_future_phase_governance(
        ...
        for phase in range(7, 13):
            ...
            if phase == 7:
                lines.extend(
                    [
                        "**Goal:** The implementation remains 67/81 canonical tools, with all 14 Phase 7 names registered but unimplemented; 15/18 are implementation-feasible and 3/18 are API-blocked under the approved contracts.",
                        "",
                    ]
                )
...
"""
    ...
    def write_manifest(self, **overrides) -> Path:
        ...
        counts = {
            "canonical": 81, "legacy": 10,
            "implemented": 67, "unimplemented": 14, "total": 91,
        }
    ...
    def test_manifest_disagreeing_with_documentation_fails(self):
        ...
        errors = VALIDATOR.validate_repository(
            root, self.write_manifest(implemented=68, unimplemented=13)
        )
        self.assertTrue(
            any("must state 68 implemented tools" in e for e in errors), errors
        )
    ...
    def test_manifest_drives_the_canonical_count_check(self):
        ...
        self.assertTrue(
            any("must be 81 canonical, 68 implemented, and 13 remaining" in e for e in errors),
            errors,
        )
    ...
    def test_stale_fallback_counts_are_reported(self):
        ...
        root = self.make_valid_repository()
        errors = VALIDATOR.validate_repository(
            root, self.write_manifest(canonical=82, implemented=68, total=92)
        )
        self.assertTrue(
...
""",
    ...
    def test_reports_missing_current_release_fact(self):
        ...
        self.write("README.md", readme.replace("81 canonical", "77 canonical"))

        errors = VALIDATOR.validate_repository(root)

        self.assertTrue(any("README.md" in error and "81 canonical" in error for error in errors), errors)
