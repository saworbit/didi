...
Didi exposes 106 canonical tools plus 10 legacy names (116 total).

Startup requires --project or DIDI_PROJECT_ROOT. Mutations expose dry_run and protected writes use confirmation_token.

Phase 7 status is PARTIAL_DELIVERY after the 2026-08-29 Godot 4.5.1 and Godot 4.7.2 feasibility gate. The implementation remains 103/106 canonical tools, with all 3 Phase 7 names registered but unimplemented. The gate found 15/18 implementation-feasible and 3/18 API-blocked under the approved contracts.
...
""",
        ...
        generic_docs = {
            ...
            "DEVELOPER_GUIDE.md": "# Developer Guide\n\nPhase 7 is PARTIAL_DELIVERY at 103/106 implemented; 15/18 names are implementation-feasible and 3/18 are API-blocked.\n",
            "INTEGRATION_GUIDE.md": "# Integration Guide\n",
            "LLM_INSTRUCTIONS.md": "# LLM Instructions\n\nStart with --project or DIDI_PROJECT_ROOT. Preview with dry_run and use confirmation_token when returned. Phase 7 is PARTIAL_DELIVERY at 103/106 implemented; all 3 names remain unimplemented and API-blocked.\n",
...
Didi v1.4.0 registers 106 canonical tool names. 103 are implemented in at least one mode; 3 remain reserved. In other words, 103 canonical tools are implemented. Ten legacy names are registered separately, for exactly 116 tools/list entries.
...
Didi exposes 106 canonical tool names plus 10 legacy names (116 registrations).
    ...
    def make_future_phase_roadmap(
        ...
        for phase, name in phase_names.items():
            ...
            if phase == 7:
                lines.extend(
                    [
                        "**Objective:** The implementation remains 103/106 canonical tools, with all 3 Phase 7 names registered but unimplemented. The feasibility gate found 15/18 implementation-feasible and 3/18 API-blocked under the approved contracts.",
                        "",
                    ]
                )
    ...
    def make_future_phase_governance(
        ...
        for phase in range(7, 13):
            ...
            if phase == 7:
                lines.extend(
                    [
                        "**Goal:** The implementation remains 103/106 canonical tools, with all 3 Phase 7 names registered but unimplemented; all 15/18 implementation-feasible names are delivered.",
                        "",
                    ]
                )
...
"""
    ...
    def test_manifest_disagreeing_with_documentation_fails(self):
        ...
        errors = VALIDATOR.validate_repository(
            root, self.write_manifest(implemented=99, unimplemented=7)
        )
        self.assertTrue(
            any("must state 99 implemented tools" in e for e in errors), errors
        )
    ...
    def test_manifest_drives_the_canonical_count_check(self):
        ...
        self.assertTrue(
            any("must be 106 canonical, 99 implemented, and 7 remaining" in e for e in errors),
            errors,
        )
...
""",
    ...
    def test_reports_missing_current_release_fact(self):
        ...
        self.write("README.md", readme.replace("106 canonical", "77 canonical"))

        errors = VALIDATOR.validate_repository(root)

        self.assertTrue(any("README.md" in error and "106 canonical" in error for error in errors), errors)
